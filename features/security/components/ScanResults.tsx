...
import { InfoIcon, ChevronRightIcon, CheckCircleIcon, RefreshCwIcon } from '../../../components/Icons';
...
export const ScanResults: React.FC<ScanResultsProps> = ({ results, onReset, onRescan }) => {
    ...
    useEffect(() => {
        // Automatically open the highest-risk category that has findings
        if (sortedRiskCategories.length > 0) {
            setOpenCategories(new Set([sortedRiskCategories[0]]));
        } else {
            setOpenCategories(new Set());
        }
        // Also reset selected finding when results change
    ...
    const summary = useMemo(() => {
        ...
        results.findings.forEach(f => {
            counts[f.severity as VulnerabilitySeverity]++;
        });
        return counts;
    }, [results.findings]);
                 ...
                 <div className="mt-4">
                    ...
                    <div className="flex gap-2 mt-1">
                        <input
                            ...
                            placeholder="e.g., 'authentication', 'payment', 'user data'"
